    ...
    for t in range(t_wait):
        for i in range(d):
            for j in range(i+1,d):
                dtheta=1*np.random.uniform(-1,1)#theta(t)-theta(t+1)
...
#samp_theta=gen_theta(1000,samp_theta,dl_sample)
test_x=gen_mcmc(1000,np.ones(d),samp_theta)
test_x=np.matrix(test_x)
config=np.dot(test_x.T,test_x)

...
for t_gd in range(t_gd_max):
    ...
    for nin in range(n_bach):
        ...
        #   Hypothesis, required proposals are smaller than d-th.
        #n_prop=0
        ...
        for l1 in range(d):
            ...
            #if(diff_E<=0):
            #    n_prop+=1
            #    grad_prop=grad_prop+(x_nin_x_shift-x_nin_l_x_shift)*np.exp(0.5*diff_E)
            gradK_nin=gradK_nin-(x_nin_x_shift-x_nin_l_x_shift)*np.exp(0.5*diff_E)/d
        """

        #if(n_prop>0):gradK_nin=grad_prop/n_prop
        #elif(n_prop==0):n_elime+=1  #this sample would be ignored, because this sample is the highest fitness around this samples
        ...
        """
        #print("n_prop=",n_prop)
        #gradK=gradK+gradK_nin/(n_bach-n_elime)

...
def main():
    ...
    x_mat_init=np.reshape(x_init,(d_x,d_y))
    x_mat_last=np.reshape(x_last,(d_x,d_y))
    mat_theta1=np.reshape(theta_model[0],(d_x,d_y))
    mat_theta2=np.reshape(theta_model[1],(d_x,d_y))

    plt.figure()
    plt.subplot(221)
    plt.imshow(x_mat_init,cmap="gray",interpolation='nearest')
    plt.title("spin mcmc init")
    plt.subplot(222)
    plt.imshow(x_mat_last,cmap="gray",interpolation='nearest')
    plt.title("spin mcmc last")
    plt.subplot(223)
    plt.imshow(mat_theta1,cmap="gray",interpolation='nearest')
    plt.title("theta 1")
    plt.subplot(224)
    plt.imshow(mat_theta2,cmap="gray",interpolation='nearest')
    plt.title("theta 2")
    plt.show()

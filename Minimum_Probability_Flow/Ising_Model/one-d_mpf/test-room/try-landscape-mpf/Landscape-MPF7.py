...
for th in theta_slice:
    ...
    for l in range(size_of_sample):
        ...
        for j in range(d):
            ...
            xl[j]*=-1
            #check of the number of neighbor in record
            p_expected_null+=prob_x_into(th,j,xl)
            p_expected_null+=prob_x_goout(th,j,xl)-1/d
            size_neighbor=len(np.where(address[0]==index_neighbor)[0]) 
            if(size_neighbor>0):
                id2=np.where(address[0]==index_neighbor)[0][0] 
                p_comin+=state_dist[id2].degree*prob_x_into(th,j,xl)
            p_goout+=prob_x_goout(th,j,xl)

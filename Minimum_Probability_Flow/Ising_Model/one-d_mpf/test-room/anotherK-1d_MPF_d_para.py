    ...
    for nin in range(n_bach):
        ...
        for l in range(d):
            if(check_list[l]==0):
                ...
                El=2*(xl_xl_plu_1*theta_model[l]+xl_min_1_xl+theta_model[(l-1+d)%d])
                El_plu_1=2*(xl_xl_plu_1*theta_model[l]+xl_plu_1_xl_pul_2+theta_model[(l+1)%d])
                gradK_nin[l]=-2*xl_xl_plu_1*( np.exp(El)/(1+np.exp(El))**2 + np.exp(El_plu_1)/(1+np.exp(El_plu_1))**2 )

...
name_tot="cv-result-MFP-smaple"+"-d"+str(d)+".dat"
f_tot=open(name_tot,"w")
sample_list=[100,200,300,400,600,800]
#sample_list=[20,40]
leave_cv=50
#leave_cv=10
for N_sample in sample_list:
    #name="HMC-MFP-smaple"+str(N_sample)+"-d"+str(d)+".dat"
    name="cv-MFP-smaple"+str(N_sample)+"-d"+str(d)+".dat"
    f=open(name,"w")
    #SAMPLING
    ...
    n_bach=len(X_sample)
    tot_sample=np.arange(n_bach)
    for n_cv in range(int(N_sample/leave_cv)):
        error_func=1000
        theta_model=np.random.uniform(0,1,d)    #Initial guess
        cv_sample=np.append(tot_sample[0:n_cv*leave_cv],tot_sample[(n_cv+1)*leave_cv:len(tot_sample)]) 
        flag=0
        for t_gd in range(t_gd_max):
            gradK=np.zeros(d)
            time_s=time.time()
            error_prev=error_func
            for nin in cv_sample:
                ...
                gradK=gradK+gradK_nin/(n_bach-n_cv)
            ...
            if(error_prev<error_func):
                ...
                time_f=time.time()
                dt=time_f-time_s
                f_tot.write(str(n_cv)+" "+str(N_sample)+" "+str(error_func)+" "+str(dt)+"\n")
                f.write(str(n_cv)+" "+str(N_sample)+" "+str(error_func)+" "+str(dt)+"\n")
                break
        if(flag==0):
            time_f=time.time()
            dt=time_f-time_s

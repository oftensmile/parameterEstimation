...
#######    MAIN    ########
name_result="result-Normal-N_sample"+str(N_sample)+".dat"
fr=open(name_result,"w")
#SAMPLING
#N_sample_list=[10,20,30,40,50,100,200,400,800,1600]

dim_list=[8,16,32,64,128]
for d in dim_list:
    ...
    check,time_s=0,time.time()
    name="Normal-dim"+str(d)+"-sample"+str(N_sample)+"fixed.dat"
    f=open(name,"w")
    ...
    for t_gd in range(t_gd_max):
        ...
        if(error_prev<error_func):
            ...
            f.write("#time="+str(dt))
            f.close()
            fr.write(str(d)+"  "+str(error_func)+" "+str(dt)+"\n")
            check=1
            break
    if(check==0):
        time_f=time.time()

...
size_sample=len(X_sample)

#MPF
beta_list=[0.1,0.2,0.4,0.5,1.0,2.0,4.0,8.0]
for beta in beta_list:
    name_result="result-Claster-N_sample"+str(N_sample)+".dat"
    fr=open(name_result,"w")
    check=0
    time_s=time.time()
    name="Cluster-Update-beta"+str(beta)+"-dim"+str(d)+"-sample"+str(N_sample)+".dat"
    f=open(name,"w")
    for n in range(size_sample):
        xn=np.copy(X_sample[n])
        if(n==0):
            X_sample_config=np.copy(claster_proposal(beta,xn))
        elif(n==1):
            X_sample_config=[X_sample_config,np.copy(claster_proposal(beta,xn))]
        else:
            #X_sample_config=np.vstack((X_sample_config,np.copy(claster_proposal(beta,xn))))
            X_sample_config.insert(n,np.copy(claster_proposal(beta,xn)))
    ...
    for t_gd in range(t_gd_max):
        ...
        for nin in range(n_bach):
            ...
            ##  Cluster porposals ##
            #propose all configulation to each sample 
            set_of_proposal=X_sample_config[nin]#claster_proposal(beta,np.copy(x_nin))
        ...
        f.write(str(beta)+" "+str(error_func)+"\n")
        if(error_prev<error_func):
            ...
            f.write("#time="+str(dt))
            f.close()
            fr.write(str(beta)+"  "+str(error_func)+" "+str(dt)+"\n")
            break
    if(check==0):
        time_f=time.time()
        dt=time_f-time_s

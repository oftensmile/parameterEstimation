    ...
    for M in data_matrix:
        ...
        for i1 in range(d):
            for i2 in range(d):
                ...
                gradK[i1+i2*d]+=-2*x[i1][i2]*x[(i1+1)%d][i2]*( np.exp(-0.5*dEi1i2)+np.exp(-0.5*dEi1pi2) )/(N_sample*d**2)
                gradK[d*d+i1+i2*d]+=-2*x[i1][i2]*x[i1][(i2+1)%d]*( np.exp(-0.5*dEi1i2)+np.exp(-0.5*dEi1i2p)  )/(N_sample*d**2)
    return gradK
if __name__ == '__main__':
    ...
    #J_data=0.1*np.random.uniform(0,1.0/d,d*d*2)
    ...
    J_model_root_mat=np.reshape(np.array(J_model_root.x),(d,2*d))
    J_data_mat=np.reshape(np.array(J_data),(d,2*d)) 
    plt.figure()
    plt.subplot(121)
    plt.imshow(np.mat(J_model_root_mat), interpolation='nearest')
    plt.colorbar()
    plt.title("J_model")
    plt.subplot(122)
    plt.imshow(np.mat(J_data_mat), interpolation='nearest')
    plt.colorbar()
    plt.title("J_data")
    plt.show()

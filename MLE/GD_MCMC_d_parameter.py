...
np.random.seed(0)
#parameter ( MCMC )
t_burn_emp, t_burn_model = 100, 40#10000, 100
t_interval = 40
#parameter ( System )
d, N_sample = 64,600 #124, 1000
N_remove=10
#parameter ( MPF+GD )
lr,eps =0.1, 1.0e-100
...
J=np.random.uniform(0,1,d) # =theta_sample
print("J_true= \n")
for i in range(d):
    print(J[i])
J_sum=np.sqrt(np.sum(J**2))
...
for t_gd in range(t_gd_max):
    ...
    error_J = np.sqrt(np.sum((theta_model-J)**2))/J_sum
    print(t_gd,np.abs(grad_likelihood),error_J)

...
np.random.seed(0)
num_sample, num_class, iteration = 10000, 10, 110
mu,sigma =  np.arange(num_class), 0.5*np.ones(num_class)
...
for t in range(iteration):
    ...
    # this line is just for checking, I guess somee of k makes prob to be zeros
    for k in range(num_class):
        print(k,'', np.sum(p[:,k]))
    for n in range(total_sample):
        print('checkk sum p',np.sum(p[n,:]))
        ganma[n,:]=p[n,:]/np.sum(np.array(p[n,:]+0.000001))
        #print(t,n,np.sum(p[n,:]))
    #M-step
    for k in range(num_class):
        print(k,'second', np.sum(ganma[:,k]))
        print(k,'second', np.sum(p[:,k]))
    for k2 in range(num_class):
        temp=np.sum(ganma[:,k2])
        print(k2,'sumof ganma',temp) 
        ...
        for k3 in range(num_class):
            alpha_est[k3]/=alpha_sum
    #print(np.sum(np.abs(alpha-alpha_est)),'',np.sum(np.abs(mu-mu_est)),'',np.sum(np.abs(sigma-sigma_est)))

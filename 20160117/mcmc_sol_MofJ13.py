...
original=[[-1*(-1)**j if i%3==0 else 1*(-1)**j for i in range(d)] for j in range(d)]#[-1,1,1-1,,...,1],[same]*(-1),[same]*(-1)^2,...
#Dameged Process

J,h=0.2,0.1
for q in range(Q):
    for t in range(nT):#Update tau by each times
        x0=np.ones((d,d))
        tau=np.ones((d,d))
        tau=mcmc_sampling_given_h(500,htrue,tau,original)
        if (t==0 and q==0): damaged=tau

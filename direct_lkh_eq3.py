...
mean_theta=mean_theta/n_sample
stack_theta=stack_theta-np.matlib.repmat(mean_theta,n_sample,1)
cov_theta=np.dot(stack_theta.T,stack_theta)

alpha=kernel_regres(stack_theta,stack_x1x2)
la,v=linalg.eig(cov_theta)
Tpca=[v[0],v[1]]
pbase=np.dot(np.matrix(Tpca),np.matrix(stack_theta).T)
base1,base2=pbase[0],pbase[1]

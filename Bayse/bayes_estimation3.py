...
def heat_exixj(id_i,id_j,ideling,waiting,sample,x=[],theta=[[]]):#note!, this function is sequence of x sampling 
    sum_expxixj=0
    x=gen_mcmc(ideling,x,theta)# this waiting time of sampling is relatively short
    for t in range(sample):
        x=gen_mcmc(waiting,x,theta)
        sum_expxixj+=np.exp(-x[id_i]*x[id_j])
    return (sum_expxixj/sample,x)
...
ideling,waiting,sample_th,sample_x,count=100,3,300,20,0
theta_mean=np.zeros((d,d))
x=np.ones(d)
for t in range(ideling+waiting*sample_th):
    for i in range(d):
        for j in range(i+d):
            ret=heat_exixj(i,j,ideling,waiting,sample_x,x,theta_est)
            heat_mean=ret[0]

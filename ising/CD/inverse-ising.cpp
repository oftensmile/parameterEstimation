...
void init_S_CD(int n_s){

	string filename="./state-L"+to_string(L)+"-T"+to_string_with_precision(T)+"-MCMC"+to_string(n_s*200)+".dat";
	ifstream myfile (filename);
	string line;
	int count = 0; int i=0,j=0; // this initialize is necessary.
	if (myfile.is_open())
...
void calc_K_data(){
	...
	for(int n=0;n<n_sample;++n){
		/********you should change filename of init_S_CD if you want to change name of this filename.********/
		string filename="./state-L"+to_string(L)+"-T"+to_string_with_precision(T)+"-MCMC"+to_string(n*200)+".dat";
...
{
	set_test_J();
	cout << "set_test_J is finished" << endl;
	calc_K_data();
	init_J_model();

...
np.random.seed(0)
####################### set parameters ########################
num_spin = 6 # nuber of spin variable
num_replica = 5 # number of reploca 
num_theta = 300 # number of epoc
num_sample = 1000 # number of sampling from true prob(theta)
num_sample_est = 100 # number of sampling from pseudo  prob(theta)
beta_min = 0.005 # inverse temp
beta_max = 0.1
...
if __name__ == "__main__":
    ...
    beta_min, beta_max = 0.005, 0.1 # inverse temp of min, max
    d_beta = float(beta_max- beta_min) / num_replica
    ...
    for epoc in range(epoc_theta):
        ...
        for t in range(num_sample_est):
            index_spin = 0
            index_spin = (index_spin + 1) % num_spin_est
            gen_samplefromgibbus(index_spin, num_spin, num_replica, beta_min,d_beta, X_est, theta_est)
            if(t % exchange_interval == 0):
                index_replica =  np.random.randint(0, num_replica - 1 ) 
        ...
        X_est =  2 * np.array(np.random.random_integers(0,1,num_spin) - 0.5)
        X_est = np.concatenate(([X],[X]), axis=0), np.concatenate(([X],[X]), axis=0)

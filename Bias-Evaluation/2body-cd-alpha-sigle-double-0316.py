...
def cd_1_single_0316(parameter=[],*args):
    J,h = parameter
    xx,xandx=args
    global sample
    M =float(len(sample)) 
    ...
    for s in sample:
        ...
        if(2*J*sas+h*sas>10):
            a,ai=0.0,1.0
        elif(2*J*sas+h*sas<-10):
            a,ai=1.0,0.0
        else:
            a=1.0/(1.0+np.exp(2*J*sas+h*sas))
            ai=1.0/(1.0+np.exp(-2*J*sas-h*sas))
        if(2*h*sas>10):b,bi=0.0,1.0
        elif(2*h*sas<-10):b,bi=1.0,0.0
        else:
            b=1.0/(1.0+np.exp(2*h*sas))
            bi=1.0/(1.0+np.exp(-2*h*sas))

        #a,b=1.0/(1.0+np.exp(2*J*sas+h*sas)),1.0/(1.0+np.exp(2*h*sas))
        #ai,bi=1.0/(1.0+np.exp(-2*J*sas-h*sas)),1.0/(1.0+np.exp(-2*h*sas))
        grad_J+=(2.0*ai+2.0*a+bi-b)
        grad_h+=(2.0*ai+bi+b)
        #grad_J += ss*(-a+b-2*c-2*d)/M
        #grad_h += sas*(-a-b-2*c)/M
        #a,b=1.0/(1.0+np.exp(2*J*ss+h*sas)), 1.0/(1.0+np.exp(2*h*sas))
        #grad_J += 4.0*ss*a
        #grad_h += 2*sas*(b+a)
...
def cd_1_single_0316_2(parameter=[],*args):
    J,h = parameter
    q1,q4=args
    q2 = (1.0-q1-q4)/2.0
    Q = [q1,q2,q2,q4]
    ...
    for i in range(4):
        ...
        #a,b=1.0/(1.0+np.exp(2*J*ss+h*sas)), 1.0/(1.0+np.exp(2*h*sas))
        #ai,bi=1.0/(1.0+np.exp(-2*J*ss-h*sas)), 1.0/(1.0+np.exp(-2*h*sas))

        #ss,sas=s[0]*s[1],s[0]+s[1]
        #a,b=1.0/(1.0+np.exp(-2*h*sas)),1.0/(1.0+np.exp(2*h*sas))
        #c,d=1.0/(1.0+np.exp(-2*sas-h*sas)),1.0/(1.0+np.exp(2*sas+h*sas))
        #grad_J += ss*(-a+b-2*c-2*d)/M
        #grad_h += sas*(-a-b-2*c)/M
        grad_J += ss*(2.0*ai+2.0*a + bi-b ) * Q[i]
        grad_h += sas*(2.0*ai+bi +b) * Q[i]
    return (grad_J,grad_h)
...
if __name__ == '__main__':
    ...
    for M in M_list:
        fname="stav-"+str(M)+"-J0-"+str(J0)+"-h0-"+str(h0)+"-cd-sing-two-update-alpha-single14.dat"
        f=open(fname,"w")
        ...
        for N in N_list:
            ...
            while(m<M):
                ...
                #Temp
                    ...
                    solv_cd_sing0316 =optimize.root(cd_1_single_0316_2,[J_init,h_init],args=args_cd).x 
                    J_cd_sing0316,h_cd_sing0316=solv_cd_sing0316[0],solv_cd_sing0316[1]
                    
                    #args_alpha050,args_alpha090= (0.50,q1,q4),(0.999,q1,q4)
                    #solv_cd_alpha050 =optimize.root(alpha_two_single,[J_init,h_init],args=args_alpha050).x   
                    #J_cd_alpha050,h_cd_alpha050 = solv_cd_alpha050[0],solv_cd_alpha050[1]
